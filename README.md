...
- **Agent Manager** (🗂️ button next to the chat header): launch Agent Mode tasks that run fully independently of the current conversation — they don't block it and aren't blocked by it, since the backend (`runRegistry.ts`) already tracked every run by a correlation id but had no registry of what was active. The panel lists every run (this session's chat, background tasks launched from here, or a Telegram/WhatsApp bot message) with live status (running/done/error), a duration counter and its latest stream message, updated in real time as the run progresses
...
- **Agent Manager** (pulsante 🗂️ accanto all'intestazione della chat): lancia task in Agent Mode completamente indipendenti dalla conversazione corrente — non la bloccano né ne sono bloccati, dato che il backend (`runRegistry.ts`) tracciava già ogni run con un id di correlazione ma non aveva un registro di cosa fosse attivo. Il pannello elenca ogni run (la chat di questa sessione, task in background lanciati da qui, o un messaggio da un bot Telegram/WhatsApp) con stato live (in esecuzione/concluso/errore), un contatore di durata e l'ultimo messaggio dello stream, aggiornati in tempo reale man mano che il run procede
