...
- **Agent Mode**: tool-calling harness with `edit_file`, `patch_file` (targeted edits without rewriting the whole file), `delete_file`, `move_file`, `create_folder`, `delete_folder` (recursive, blocked from touching the project root or anything outside it), `read_file`, `search_codebase`, `semantic_search` (local embedding-based search via Ollama's `nomic-embed-text` — finds code by meaning, e.g. "where do we handle terminal disconnection", when the exact string isn't known) — Settings now shows a ready/not-ready badge for it next to the installed Ollama models list, instead of the only signal being a failed tool call the first time an agent tries to use it, `get_repo_map` (compact AST map + file dependencies), `get_diagnostics` (real static analysis — the TypeScript Compiler API for `.ts`/`.tsx`/`.js`/`.jsx`, `mypy`/`py_compile` for `.py`, `cargo check` for `.rs`, `go vet` for `.go`, run through the user's real login shell so pyenv/rustup-installed tools are found, same fix as the integrated terminal), `ocr_image`, `browser_navigate`/`browser_screenshot`/`browser_eval`/`browser_click`/`browser_smart_locate`/`browser_solve_cloudflare` (Browser Agent — Electron window by default, optional separate stealth Chromium/Chrome process), `manage_git`, `save_memory`/`forget_memory` (a rule can now actually be removed once it's wrong or outdated, not just overwritten by the same key or left injected forever), `publish_to_github`, `run_and_verify_tests`, `invoke_subagent` (read-only sub-agents, with diagnostics, OCR and read-only browser access), `web_search`, `fetch_webpage`
...
- **Deep Reasoning**: a genuine two-level Monte Carlo Tree Search — N independent branches investigate the real code in parallel and each proposes a solution; every proposal is actually applied transiently (always reverted right after) and scored with a real type-check — partial-credit scoring, not a text opinion; the most promising-but-imperfect branches get refined with a second pass based on real feedback and re-scored, then the highest-scoring node wins. The "Supreme Court" mode (3 judges + syntactic screening + an LLM judge vote) remains available as a lighter option in `deepReasoning.ts`. A third **"Automatic"** option (besides On/Off) routes to it without asking the user every time: a local heuristic (`difficultyRouter.ts` — prompt length, number of distinct files mentioned, multi-step connectors like "then"/"after that", high-risk keywords such as "refactor"/"architecture"/"race condition" vs. low-risk ones like "rename"/"fix typo") decides per-task whether the extra cost is worth it, with the decision shown transparently in the stream ("task assessed as complex/simple") — no extra LLM call is spent just to decide whether to make one
- **Agent Manager** (🗂️ button next to the chat header): launch Agent Mode tasks that run fully independently of the current conversation — they don't block it and aren't blocked by it, since the backend (`runRegistry.ts`) already tracked every run by a correlation id but had no registry of what was active. The panel lists every run (this session's chat, background tasks launched from here, or a Telegram/WhatsApp bot message) with live status (running/done/error/cancelled), a duration counter and its latest stream message, updated in real time as the run progresses. Each running task has a **⏹ Stop** button — cooperative cancellation checked at safe boundaries (before the next agent iteration, between MCTS phases) rather than a hard kill, since no real `AbortController` threads through every LLM provider client; nothing already written to disk is undone, but nothing further gets written either. Background tasks can also be launched in **Plan Mode** (investigate + propose only, no writes) for anything you'd rather review before it touches files unattended
...
- **Agent Mode**: harness a tool-calling con `edit_file`, `patch_file` (modifica mirata senza riscrivere l'intero file), `delete_file`, `move_file`, `create_folder`, `delete_folder` (ricorsivo, bloccato dal toccare la root del progetto o qualunque cosa al suo esterno), `read_file`, `search_codebase`, `semantic_search` (ricerca locale basata su embedding via Ollama `nomic-embed-text` — trova codice per significato, es. "dove gestiamo la disconnessione del terminale", quando non si conosce la stringa esatta) — le Impostazioni mostrano ora un badge pronto/non pronto per questo tool accanto alla lista dei modelli Ollama installati, invece che l'unico segnale fosse una chiamata a tool fallita la prima volta che un agente prova ad usarlo, `get_repo_map` (mappa AST compatta + dipendenze tra file), `get_diagnostics` (vera analisi statica — TypeScript Compiler API per `.ts`/`.tsx`/`.js`/`.jsx`, `mypy`/`py_compile` per `.py`, `cargo check` per `.rs`, `go vet` per `.go`, eseguiti passando per la vera shell di login dell'utente così i tool installati via pyenv/rustup vengono trovati, stesso fix già usato per il terminale integrato), `ocr_image`, `browser_navigate`/`browser_screenshot`/`browser_eval`/`browser_click`/`browser_smart_locate`/`browser_solve_cloudflare` (Browser Agent — finestra Electron per default, opzionale processo Chromium/Chrome separato in modalità stealth), `manage_git`, `save_memory`/`forget_memory` (una regola può ora essere davvero rimossa quando risulta sbagliata o superata, non solo sovrascritta dalla stessa chiave o iniettata per sempre), `publish_to_github`, `run_and_verify_tests`, `invoke_subagent` (sub-agenti in sola lettura, con accesso anche a diagnostica, OCR e browser read-only), `web_search`, `fetch_webpage`
...
- **Deep Reasoning**, attivata dalla stessa modalità, è un vero Monte Carlo Tree Search a due livelli: N rami indipendenti investigano il codice reale in parallelo e propongono una soluzione ciascuno; ogni proposta viene DAVVERO applicata in modo transitorio (sempre ripristinata subito dopo) e valutata con una vera diagnostica di tipo — punteggio a *partial credit*, non un voto testuale; i rami più promettenti ma imperfetti vengono raffinati con un secondo giro basato sul feedback reale e ri-valutati, poi vince il nodo con il punteggio più alto in assoluto. La modalità "Supreme Court" (3 giudici + screening sintattico + voto di un giudice LLM) resta disponibile come funzione più leggera in `deepReasoning.ts`. Una terza opzione **"Automatico"** (oltre a On/Off) vi instrada senza dover chiedere ogni volta all'utente: un'euristica locale (`difficultyRouter.ts` — lunghezza del prompt, numero di file distinti menzionati, connettori multi-passo come "poi"/"successivamente", parole chiave ad alto rischio come "refactor"/"architettura"/"race condition" contro altre a basso rischio come "rinomina"/"correggi il typo") decide per ogni task se vale la pena il costo extra, mostrando la decisione in modo trasparente nello stream ("task valutato complesso/semplice") — nessuna chiamata LLM aggiuntiva viene spesa solo per decidere se farne una
- **Agent Manager** (pulsante 🗂️ accanto all'intestazione della chat): lancia task in Agent Mode completamente indipendenti dalla conversazione corrente — non la bloccano né ne sono bloccati, dato che il backend (`runRegistry.ts`) tracciava già ogni run con un id di correlazione ma non aveva un registro di cosa fosse attivo. Il pannello elenca ogni run (la chat di questa sessione, task in background lanciati da qui, o un messaggio da un bot Telegram/WhatsApp) con stato live (in esecuzione/concluso/errore/interrotto), un contatore di durata e l'ultimo messaggio dello stream, aggiornati in tempo reale man mano che il run procede. Ogni task in esecuzione ha un pulsante **⏹ Stop** — interruzione cooperativa controllata ai confini sicuri (prima della prossima iterazione dell'agente, tra una fase e l'altra dell'MCTS) invece di un kill immediato, dato che nessun `AbortController` reale attraversa tutti i client dei vari provider LLM: nulla di già scritto su disco viene annullato, ma non viene scritto altro. I task in background possono anche partire in **Modalità Piano** (sola investigazione + proposta, nessuna scrittura) per tutto ciò che preferisci revisionare prima che tocchi i file senza presidio
