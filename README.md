...
- **Native audio conversation** (WebRTC, OpenAI Realtime) — a real microphone-to-microphone conversation with a model that natively supports audio, selectable from its own menu group; replaces an earlier Moshi/LiveKit integration removed because it only understood English.
- **Attachments** — a paperclip button to attach images or plain-text documents (code, `.md`, `.json`, `.csv`, etc.) from anywhere on disk, not just files already inside the open project.
...
- **Agent Mode**: tool-calling harness with `edit_file`, `patch_file` (targeted edits without rewriting the whole file), `read_file`, `search_codebase`, `get_repo_map` (compact AST map + file dependencies), `get_diagnostics` (real type-checking via the TypeScript Compiler API), `browser_navigate`/`browser_screenshot`/`browser_eval`/`browser_click` (Browser Agent — a real Chromium window for visual testing), `manage_git`, `save_memory`, `publish_to_github`, `run_and_verify_tests`, `invoke_subagent` (read-only sub-agents, with diagnostics and read-only browser access), `web_search`, `fetch_webpage`
...
- **Checkpoint/rewind**: every agent run that modifies files (`edit_file`/`patch_file`) creates an automatic checkpoint; a button on the final message reverts ALL changes from that run as one atomic unit. Doesn't cover `run_terminal_command` side effects, too varied to track reliably without a full sandbox
...
- **Conversazione vocale nativa** (WebRTC, OpenAI Realtime) — una vera conversazione microfono-a-microfono con un modello che supporta audio nativo, selezionabile dal suo gruppo dedicato nel menu; sostituisce una precedente integrazione Moshi/LiveKit rimossa perché capiva solo l'inglese.
- **Allegati** — un bottone graffetta per allegare immagini o documenti di testo semplice (codice, `.md`, `.json`, `.csv`, ecc.) da qualunque punto del disco, non solo file già dentro al progetto aperto.
...
- **Agent Mode**: harness a tool-calling con `edit_file`, `patch_file` (modifica mirata senza riscrivere l'intero file), `read_file`, `search_codebase`, `get_repo_map` (mappa AST compatta + dipendenze tra file), `get_diagnostics` (type-checking reale via TypeScript Compiler API), `browser_navigate`/`browser_screenshot`/`browser_eval`/`browser_click` (Browser Agent — finestra Chromium reale per test visivi), `manage_git`, `save_memory`, `publish_to_github`, `run_and_verify_tests`, `invoke_subagent` (sub-agenti in sola lettura, con accesso anche a diagnostica e browser read-only), `web_search`, `fetch_webpage`
...
- **Checkpoint/rewind**: ogni run dell'agente che modifica file (`edit_file`/`patch_file`) crea un checkpoint automatico; un pulsante sul messaggio finale permette di annullare TUTTE le modifiche di quel run come unità atomica, ripristinando i file al loro stato precedente. Non copre gli effetti di `run_terminal_command`, troppo vari per essere tracciati in modo affidabile senza una sandbox completa
